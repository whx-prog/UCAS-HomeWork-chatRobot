...
fun queryFromDatabase(
    ...
    val isNext_Last_Week = if(inputDayOfWeek > 7) 1 else if(inputDayOfWeek<=0) -1 else 0
    select {
        (beginYear eq TimeProviderService.currentSemesterBeginYear) and
                ...
                (whichDayOfWeek eq if((inputDayOfWeek+7)%7==0) 7 else (inputDayOfWeek+7)%7)
    }.filter {
        val weeks = it[weekPeriod].split(" ").map { w -> w.toInt() }
        weeks.contains(TimeProviderService.currentWeekPeriod.getOrDefault(belongingSchool, -1) + isNext_Last_Week)
...
suspend fun FriendMessageEvent.queryCourse(matchResult: MatchResult) {
    val whichDay = when {
        ...
        matchResult.groupValues[3] == "后" -> TimeProviderService.currentTimeStamp.dayOfWeek.value + 2
        matchResult.groupValues[3] == "昨" -> TimeProviderService.currentTimeStamp.dayOfWeek.value - 1
        matchResult.groupValues[3] == "前" -> TimeProviderService.currentTimeStamp.dayOfWeek.value - 2
        else -> {
            subject.sendMessage("你还没有输入想查询的日期或格式有误！\n 例子：今日课程 或 明日课程 或 昨天课程 或 周几课程")
            return
        }
    }
